...
##  Preliminaries<a name="secPreliminaries">(sec 1)</a>
...
Here $0\leq e_{1}\lt e_{2}\lt \dots\lt e_{k}\leq n$ and $\abs{f_{i}}\leq2^{\tau}$.
</div><br/>
...
[Final remarks in {% cite  KOIRAN2019 %}]
...
for the case $\frac{p}{q}\gt 0$ suffices, because otherwise we can compute
the sign of $f(-x)$ at $-\frac{p}{q}$. Note that $f(-x)$ is also
$(n,k,\tau)$-nomial if $f(x)$ is. Now we can also assume that $\frac{p}{q}\gt 1$.
...
Suppose $f=ax^{n}+bx^{m}+c$ with $n\gt m$. Write $f$ as $f=x^{m}g+c$
...
*  $m$ is large. More specifically, $m\gt \frac{C_{1}\log n\cdot\tau^{2}+2\tau}{\log(1+\epsilon)}$.
In this case, we have: $\abs{(\frac{p}{q})^{m}g}\gt 2^{m\log(1+\epsilon)-C_{1}\log n\cdot\tau^{2}}\gt 2^{2\tau}$.
...
*  $n$ is large compared to $m$. More specifically, $n\gt \frac{2\tau+\log(1+(1+\epsilon)^{m})}{\log(1+\epsilon)}$.
...
If $\frac{p}{q}=1+\epsilon$ with $\epsilon\gt 1$ in [lem 5](#lemsignchecking),
