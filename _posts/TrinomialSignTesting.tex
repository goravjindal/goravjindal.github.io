...
\usepackage{babel}
\usepackage[bibstyle=authoryear,citestyle=alphabetic,backend=bibtex8,bibencoding=ascii, language=auto, style=alphabetic, sorting=nyt,  natbib=true]{biblatex}
...
\begin{document}
\include{macros}\input{authors.tex}
\title{Trinomial Sign Checking}
\author{Gorav Jindal}
\maketitle
\begin{abstract}
...
\begin{problem}
\label{prob:signchecksparse}Given an $(n,k,\tau)$-nomial $f$ and
a rational number $\frac{p}{q}$ with $\logb{p,q}\leq s$, determine
the sign of $f(\frac{p}{q})$.
\end{problem}

In \ref{prob:signchecksparse}, the input size can be assumed to be
$O(k\cdot(\log n+\tau)+s)$.

\section{Trinomials}

Here, we are interested in \ref{prob:signchecksparse} in the case
when $k=3$. We call $(n,3,\tau)$-nomial be be trinomials. First
we recall the following \ref{prop:binomiallarge} from \cite{KOIRAN2019}.
\begin{prop}[Proposition 4 in \cite{KOIRAN2019}]
\label{prop:binomiallarge}Let $g(x)=a_{1}x^{\beta}+a_{2}$ where
...
\begin{lem}[Final remarks in \cite{KOIRAN2019}]
\label{lem:signevalbiomial}Let $g(x)=a_{1}x^{\beta}+a_{2}$ where
$a_{1},a_{2}$ are two nonzero integers and $\beta$ is a positive
integer. We can compute the sign of $g$ at a rational point $\frac{p}{q}$
where $p$, $q$ are positive integers, in polynomial time.
\end{lem}

First we make an easy observation that solving \ref{prob:signchecksparse}
...
easy \ref{lem:signchecking}.
\begin{lem}
\label{lem:signchecking}Let $\frac{p}{q}=1+\epsilon$ (with $\epsilon\leq1$)
...
assume that $g(\frac{p}{q})\neq0$. By using \ref{prop:binomiallarge},we
obtain that
...
\begin{casenv}
\item $m$ is large. More specifically, $m>\frac{C_{1}\log n\cdot\tau^{2}+2\tau}{\log(1+\epsilon)}$.
...
which can be computed in polynomial time (by using \ref{lem:signevalbiomial}).
\item $m$ is small. More specifically, $m\leq\frac{C_{1}\log n\cdot\tau^{2}+2\tau}{\log(1+\epsilon)}$.
Now we have two sub-cases.
\begin{casenv}
\item $n$ is large compared to $m$. More specifically, $n>\frac{2\tau+\log(1+(1+\epsilon)^{m})}{\log(1+\epsilon)}$.
In this case, leading monomial $ax^{n}$ determines the sign of $f$.
Therefore $\sgn(f(\frac{p}{q}))=\sgn(a)$. Hence sign can be computed
easily.
\item $n$ is comparable to $m$. More specifically, $n\leq\frac{2\tau+\log(1+(1+\epsilon)^{m})}{\log(1+\epsilon)}$.
...
\end{proof}
\begin{rem}
If $\frac{p}{q}=1+\epsilon$ with $\epsilon>1$ in \ref{lem:signchecking},
